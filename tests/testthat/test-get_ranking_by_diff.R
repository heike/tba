...
#     get_ranking_by(score, score-foulPoints)
  expect_equal(round(ranking$`rating_diff(score)`[1:5],1), c(35.8, 17.4, 16.8, 15.3, 11.3))

  ranking_shift <-  dplyr::filter(match_details, comp_level == "qm") %>% # get the qualifying matches
    get_ranking_by_diff(score-10)
  # for the score difference, the values don't change when the score shifts systematically:
  expect_equal(round(ranking_shift$`rating_diff(score - 10)`,2), round(ranking$`rating_diff(score)`,2))
  
  ranking_glm <-  dplyr::filter(match_details, comp_level == "qm") %>% # get the qualifying matches
    get_ranking_by_diff(score, method="loglinear")
  expect_equal(round(ranking_glm$`rating_diff(score)`[1:5],1), c(1.2, 1.2, 1.0, 0.7, 0.4))
  ...
  expect_error(get_ranking_by_diff(match_details %>% select(-match_number), score))
  
  # Warning when match_numbers alone are not unique
  expect_error(get_ranking_by_diff(match_details %>% select(-set_number), score))
